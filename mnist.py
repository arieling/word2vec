...
flags.DEFINE_string("save_path", "temp/", "Directory to write the model.")
flags.DEFINE_string(
    "train_data", "text8",
...
def main():
  ...
  with tf.Graph().as_default(), tf.Session() as session:
    if FLAGS.train:
        ...
        ckpt = tf.train.get_checkpoint_state("temp/checkpoint")
        saver.restore(session, "temp/model.ckpt-2265034")
        if FLAGS.interactive:
          # E.g.,
          model.analogy(b'france', b'paris', b'russia')
          model.nearby([b'proton', b'elephant', b'maxwell'])
...
def predectResult(source, to, predect):
    opts = Options()
    with tf.Graph().as_default(), tf.Session() as session:
        model = Word2Vec(opts, session)
        model.read_analogies()
        ...
        ckpt = tf.train.get_checkpoint_state("temp/checkpoint")
        saver.restore(session, "temp/model.ckpt-2265034")
        c = model.analogy(source.encode('UTF-8'), to.encode('UTF-8'), predect.encode('UTF-8'))
    return c

if __name__ == "__main__":
    main()
    c = predectResult('france','paris','russia')
    print(c)
